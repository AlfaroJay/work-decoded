...
Invoice Number `fldbhgfIaEE1jPwrP` (**formula**, `WD-YYYY-NNN`, built from `Invoice Seq` + created year — never hand-edited; INV-1 passes it to Square so both systems show the same number; the sequence is continuous, doesn't reset yearly, and gaps from deleted rows are normal) · **Invoice Seq** `fldJZ7LpLpqkXq9sD` (autonumber backing the formula — guarantees uniqueness even when several invoices are created in the same second) · Client `fldwwI5yYPpRePJCp` · Session `fldTQbQFOHDSXbzAB` · Amount `fld7zJWGd68pYOZSF` · Status `fldCtwEix0czYxvBx` (Draft/Pending/Sent/Paid) · Invoice Date `fld5iZIvMM505ulyz` · Due Date `fldF0su5R9sKOmawO` (set by INV-0 = session date) · Square Invoice ID `fldm827e6WFfmGC31` · Package Type `fldTakl3KFTLY8OCN` · **Package Code Issued** `fldzvvymDqC8b5K8D` (checkbox; INV-2 dedup guard — set by the Zap when a code is issued).
...
| SS-1v2: Accepted → Calendar + SMS | `366933187` (v4) | **LIVE** | On Accept: Google Calendar event + Meet, saves Meet Link **and Google Calendar Event ID** back to the Session (the Event ID is the join key the post-session Zap depends on), sends SS-1 SMS. Filter: `Status = Accepted` AND `Meet Link does not exist` → fires once. |
...
| INV-2: Invoice Paid → Package Code + Email | `367640419` (v3) | **LIVE** | Paid **pack** invoice (filter: `Package Type contains "pack"` AND `Package Code Issued is false`) → generate `PKG-XXXX-XXXX` (Code step), create the Package Codes row with Sessions Purchased, **mark the invoice issued**, email the code to the client, log the event. Single-session invoices never produce codes. |
...
| Post-Session → Consultant Feedback Reminder | `363446853` (v8) | **LIVE** | Calendar event ends → find the Session → mint a **signed** feedback link (Code step, §8) → email the consultant. Skips if feedback was already submitted (grace-window filter). |
...
- **Invoice never reached the client:** Zap History → INV-1 → read the error. Missing Due Date → fix the row; it sends on the next poll. Errored run (Square error of any kind) → **do not replay**: INV-1 reserves a Square order keyed to the Airtable row, and after an errored run that reservation is unusable — replays fail forever with "order ID is not valid." Delete the stuck Invoices row, create a fresh one with the same client/session/amount/due date, and it sends within ~2 minutes under a new number. Then check Square → Invoices → Drafts for an orphaned draft from the failed attempt and delete it.
